...
export default function Reports({ month, year }) {
  ...
  return (
    <div>
      <h2 style={{ marginBottom: 16, color: '#001529' }}>Análise de Despesas</h2>
      ...
      {transactions.length === 0 ? (
        ...
        <div style={{ textAlign: 'center', padding: 50, background: '#fff', borderRadius: 8 }}>
            <Empty description="Você ainda não cadastrou Despesas." />
            <p>Cadastre uma saída de dinheiro para ver o gráfico.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        
          <Card title="Evolução Financeira (Mês a Mês)" variant="borderless" style={{ borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
             <HistoryChart transactions={transactions} />
          </Card>

          <Row gutter={24}>
            <Col span={12}>
              <Card title="Distribuição de Gastos" variant="borderless" style={{ height: '100%', minHeight: 400, borderRadius: 8 }}>
