...
|---|---|---|---|---|---|---|
| OAuth Authorization Code + PKCE | RF-MCP-014/US4 | Fundação OAuth | T017, T023 | OpenIddict em `Program.cs` e `/oauth/authorize`/`/oauth/token`, PKCE S256 obrigatório | Fluxo real ChatGPT → Render validado com authorization code, PKCE S256 e resource | Validado produção |
...
| Discovery + Protected Resource Metadata | US4 | Fundação segura | T017, T022 | OpenIddict discovery + `McpMetadataController` | Discovery e metadata HTTPS Render 200 no fluxo real do ChatGPT | Validado produção |
| Scope `finflow.read` | RF-MCP-015 | Política MCP | T019 | `McpRead` exige claim de scope | Token real do ChatGPT chamou somente tools com `finflow.read`; testes negativos locais | Validado produção |
| Audience/resource | RF-MCP-014 | OAuth | T017 | Validação e principal usam resource canônica `/mcp` | Discovery/OAuth e chamada MCP reais usaram a audiência canônica `/mcp` | Validado produção |
...
| Reporting policy | RF-MCP-009 | Fase 3 | T024, T029 | `ReportingPolicy.OperationalPredicate` compartilhada pelo módulo MCP | Teste PostgreSQL isolado confirma liquidações e ajustes fora dos agregados operacionais | Validado produção |
