...
|---|---|---|---|---|---|---|
| OAuth Authorization Code + PKCE | RF-MCP-014/US4 | Fundação OAuth | T017, T023 | Configuração OpenIddict parcial em `Program.cs` | Nenhum fluxo executável | Pendente |
| Refresh rotation e revogação | RF-MCP-014 | OAuth | T017–T018 | Migration OpenIddict criada | Sem teste | Pendente |
| Discovery + Protected Resource Metadata | US4 | Fundação segura | T017, T022 | Não implementado | Sem teste HTTP | Pendente |
...
| Allowlist exata de 8 tools | RF-MCP-001 | Superfície MCP | T020 | Apenas 3 tools registradas | `tools/list` não validado | Pendente |

## Checkpoint

O documento congela o estado real antes da continuação: a fundação OAuth e o exit gate P1 ainda não estão completos. As cinco tools P2 continuam deliberadamente sem implementação.
